...
var user = ls.get('otho_user', null), saved = ls.get('otho_saved', []),
    leads = ls.get('otho_leads', []), subs = ls.get('otho_subs', []),
    tray = [], pending = null, otpCode = null, gal = [], galI = 0;
...
function ring(score, size) {
  ...
  return '<div class="ring" style="width:' + s + 'px;height:' + s + 'px">'
    + '<svg width="' + s + '" height="' + s + '"><circle class="trk" cx="' + s / 2 + '" cy="' + s / 2 + '" r="' + rr + '"/>'
    + '<circle class="val" cx="' + s / 2 + '" cy="' + s / 2 + '" r="' + rr + '" stroke="' + scoreColor(score) + '" '
    + 'stroke-dasharray="' + c.toFixed(1) + '" stroke-dashoffset="' + (RM ? off.toFixed(1) : c.toFixed(1)) + '" data-ring="' + off.toFixed(1) + '"/></svg>'
    + '<b style="color:' + scoreColor(score) + '">' + score.toFixed(1) + '</b></div>';
}
function svgPlan(u) {
  ...
  return s + '<g stroke="var(--card)" stroke-width="6"><line x1="240" y1="175" x2="240" y2="205"/><line x1="300" y1="220" x2="330" y2="220"/>'
    ...
    + ' SQ FT · SUPER BUILT-UP ' + u.sba.toLocaleString('en-IN') + ' SQ FT · INDICATIVE</text>'
    + '<g stroke="var(--ink-3)"><line x1="470" y1="392" x2="570" y2="392"/><line x1="470" y1="388" x2="470" y2="396"/><line x1="570" y1="388" x2="570" y2="396"/></g>'
...
function svgMap(l) {
  ...
  return '<svg viewBox="0 0 1080 380" role="img" aria-label="Schematic map" style="display:block;width:100%;height:auto">'
    + '<rect width="1080" height="380" fill="var(--sunk)"/><g stroke="var(--line)">' + g + '</g>'
    ...
    + '<circle cx="520" cy="170" r="15" fill="var(--brass)"><animate attributeName="r" values="15;19;15" dur="2.6s" repeatCount="indefinite"/></circle>'
    + '<circle cx="520" cy="170" r="27" fill="none" stroke="var(--brass)" stroke-width="2" opacity=".45"><animate attributeName="r" values="27;36;27" dur="2.6s" repeatCount="indefinite"/><animate attributeName="opacity" values=".45;0;.45" dur="2.6s" repeatCount="indefinite"/></circle>'
    + '<text x="520" y="214" text-anchor="middle" font-family="sans-serif" font-size="14" font-weight="600" fill="var(--ink)">' + esc(l.n) + '</text>'
...
/* ---------- gate ---------- */
function gated(a, l) { return 'data-gate="' + esc(a) + '" data-glabel="' + esc(l || '') + '"'; }
function requireUser(a, l) { if (user && user.verified) { runAction(a, l); return; } pending = { action: a, label: l }; openModal(1); }
function runAction(a) {
  var p = a.split(':');
  if (p[0] === 'visit') { location.hash = '#/visit'; toast('Verified — pick a slot below.'); }
  else if (p[0] === 'plan') toast('Prototype — a live build emails the plan set to ' + (user ? user.email : 'you') + '.');
  else if (p[0] === 'price') { render(); toast('Unit pricing unlocked.'); }
  else if (p[0] === 'save') { toggleSave(+p[1]); render(); }
  else toast('Verified.');
}
...
function openModal(step) {
  ...
  ov.classList.add('on');
  if (step === 1) {
    mo.innerHTML = '<button class="x" id="mx" aria-label="Close">×</button><div class="steps"><i class="on"></i><i></i></div>'
      + '<h3 id="ovT">One step, then it\'s yours</h3>'
      + '<p style="color:var(--ink-2);font-size:14px;margin:0 0 16px">'
      + esc(pending && pending.label ? pending.label : 'Verify once and everything unlocks — floor plans, unit pricing, site visits.')
      ...
      + ['Salaried — IT / GCC','Salaried — other','Business owner','Doctor / healthcare','Government service','NRI','Investor','Other'].map(function (o) { return '<option>' + o + '</option>'; }).join('') + '</select></div>'
      + '<div><label for="uTime">PLANNING TO BUY</label><select id="uTime">'
      + ['Within 3 months','In 3–6 months','In 6–12 months','Just researching'].map(function (o) { return '<option>' + o + '</option>'; }).join('') + '</select></div></div>'
      + '<div><label for="uBud">BUDGET · <span id="uBudV">₹2.50 Cr</span></label><input type="range" id="uBud" min="5000000" max="60000000" step="500000" value="25000000"></div>'
      + '<div class="consent-row"><input type="checkbox" id="uConsent"><label for="uConsent">I agree to be contacted by OTHO about my enquiry on phone, WhatsApp and email, between 09:00 and 21:00. I can withdraw at any time. My details will not be shared with any developer or broker.</label></div>'
      + '<p class="err" id="uErr"></p><button class="btn wide" id="uNext" style="margin-top:10px">Send OTP</button>'
      + '<p class="note" style="text-align:center">Prototype — no real SMS is sent.</p>';
    document.getElementById('uBud').addEventListener('input', function () { document.getElementById('uBudV').textContent = '₹' + (+this.value / 1e7).toFixed(2) + ' Cr'; });
    document.getElementById('uNext').addEventListener('click', step1);
    document.getElementById('mx').addEventListener('click', closeModal);
    setTimeout(function () { var e = document.getElementById('uPhone'); if (e) e.focus(); }, 60);
  } else {
    mo.innerHTML = '<button class="x" id="mx" aria-label="Close">×</button><div class="steps"><i class="on"></i><i class="on"></i></div>'
      + '<h3 id="ovT">Enter the code</h3><p style="color:var(--ink-2);font-size:14px;margin:0 0 4px">Sent to +91 ' + esc(user.phone) + '.</p>'
      ...
      + '<p class="err" id="oErr"></p><button class="btn wide" id="oVerify">Verify and continue</button>'
      + '<button class="btn ghost wide" id="oBack" style="margin-top:8px">Change number</button>';
    var b = mo.querySelectorAll('[data-otp]'); b[0].focus();
    Array.prototype.forEach.call(b, function (el, i) {
      el.addEventListener('input', function () { this.value = this.value.replace(/\D/g, ''); if (this.value && b[i + 1]) b[i + 1].focus(); if (i === 5) step2(); });
      el.addEventListener('keydown', function (e) { if (e.key === 'Backspace' && !this.value && b[i - 1]) b[i - 1].focus(); });
    });
...
function step1() {
  var ph = document.getElementById('uPhone').value.replace(/\D/g, ''), em = document.getElementById('uEmail').value.trim(), er = document.getElementById('uErr');
  ...
  user = { phone: ph, email: em, profession: document.getElementById('uProf').value, timeline: document.getElementById('uTime').value,
           budget: +document.getElementById('uBud').value, verified: false, at: new Date().toISOString() };
  otpCode = String(Math.floor(100000 + Math.random() * 900000));
  openModal(2);
}
function step2() {
  var b = document.querySelectorAll('[data-otp]'), code = '';
  Array.prototype.forEach.call(b, function (x) { code += x.value; });
  if (code.length < 6) return;
  if (code !== otpCode) { document.getElementById('oErr').textContent = 'That code does not match. Try again.'; return; }
  user.verified = true; ls.set('otho_user', user);
  leads.push({ phone: user.phone, email: user.email, profession: user.profession, timeline: user.timeline,
               budget: user.budget, source: (pending && pending.action) || 'direct', at: new Date().toISOString() });
  ls.set('otho_leads', leads);
  closeModal();
  var p = pending; pending = null; render();
  if (p) runAction(p.action); else toast('Verified. Everything is unlocked.');
}
function leadScore(l) {
  var t = { 'Within 3 months': 3, 'In 3–6 months': 2, 'In 6–12 months': 1, 'Just researching': 0 }[l.timeline] || 0;
  var b = l.budget >= 30000000 ? 2 : (l.budget >= 15000000 ? 1 : 0), n = t + b;
  return n >= 4 ? ['HOT','lb-hot'] : (n >= 2 ? ['WARM','lb-warm'] : ['COLD','lb-cold']);
}

/* ---------- shared blocks ---------- */
function pcard(p) {
  var x = m(p), inTray = tray.indexOf(p.id) > -1;
  return '<div class="card pad0 lift"><a href="#/project/' + p.id + '" style="text-decoration:none;color:inherit;display:block">' + pvis(p)
    + '<div class="card-body"><div class="pcard-top"><div><div class="meta">' + esc(p.loc.toUpperCase()) + '</div>'
    + '<h3>' + esc(p.n) + '</h3><div class="meta" style="margin:2px 0 0">' + esc(p.dev) + '</div></div>' + ring(p.score, 54) + '</div>'
    + '<p>' + esc(p.sum) + '</p>'
    ...
    + '<div class="tagrow">' + p.am.slice(0, 4).map(function (i) { return '<span class="tag">' + esc(AMEN[i]) + '</span>'; }).join('') + '</div></div></a>'
    + '<div class="card-body" style="border-top:1px solid var(--line);padding-top:11px;display:flex;gap:8px;flex-wrap:wrap">'
    + '<button class="save" ' + gated('save:' + p.id, 'Save ' + p.n + ' to your shortlist.') + ' aria-pressed="' + (isSaved(p.id) ? 'true' : 'false') + '">'
    + (isSaved(p.id) ? '★ Saved' : '☆ Save') + '</button>'
...
function newsletter() {
  return '<div class="nl"><div><h3>The monthly Hyderabad market note</h3>'
    ...
    + '<p id="nlMsg" style="grid-column:1/-1;margin:0;font-size:13.5px;opacity:.88;min-height:1.2em"></p></div>';
}
function calcHTML(p) {
...
function proseBody(b) {
  return b.map(function (x) {
    if (x[0] === 'p') return '<p>' + esc(x[1]) + '</p>';
    if (x[0] === 'h') return '<h3>' + esc(x[1]) + '</h3>';
    if (x[0] === 'q') return '<div class="pullq">' + esc(x[1]) + '</div>';
    if (x[0] === 'l') return '<ul>' + x[1].map(function (i) { return '<li>' + esc(i) + '</li>'; }).join('') + '</ul>';
    return ''; }).join('');
}
function legalShell(t, l, b) {
  return '<div class="wrap"><section><p class="eyebrow">LEGAL</p><h1>' + esc(t) + '</h1><p class="sub">' + esc(l) + '</p><div class="prose">'
    + b.map(function (x) {
      if (x[0] === 'p') return '<p>' + esc(x[1]) + '</p>';
      if (x[0] === 'h') return '<h3>' + esc(x[1]) + '</h3>';
      if (x[0] === 'l') return '<ul>' + x[1].map(function (i) { return '<li>' + esc(i) + '</li>'; }).join('') + '</ul>';
      return ''; }).join('')
    + '</div><p class="note">Prototype text. A live site requires this reviewed by counsel before launch.</p></section></div>';
}
...
V.home = function () {
  ...
  return '<section class="hero"><div class="hero-media">'
    ...
    + '<section class="rv"><h2>Highest rated right now</h2><p class="lede">Scored on land share, loading, delivery record, water, density and measured commute times.</p>'
...
V.search = function () {
  return '<div class="wrap"><section><p class="eyebrow">SEARCH</p><h1>Find it by what actually matters</h1>'
    ...
    + '<div style="margin-bottom:14px"><label for="qBud">MAXIMUM BUDGET · <span id="qBudV">₹6.00 Cr</span></label><input type="range" id="qBud" min="8000000" max="60000000" step="500000" value="60000000"></div>'
    + '<div><label>AMENITIES — MUST HAVE ALL SELECTED</label><div class="chips" id="qAm">'
    + AMEN.map(function (a, i) { return '<button class="chip" data-am="' + i + '" aria-pressed="false">' + esc(a) + '<span class="n"></span></button>'; }).join('') + '</div></div>'
    + '<button class="btn ghost sm" id="qClear">Clear all filters</button></div>'
    + '<p class="note" id="qCount" style="margin-bottom:16px"></p><div class="grid g2" id="qList"></div></section></div>';
};
...
V.projects = function () {
  return '<div class="wrap"><section><p class="eyebrow">REVIEWED PROJECTS</p><h1>All ' + P.length + ' projects we cover</h1>'
    ...
    + '<div class="chips" id="locChips"><button class="chip" data-loc="" aria-pressed="true">All<span class="n">' + P.length + '</span></button>'
    + L.map(function (l) { var c = P.filter(function (p) { return p.loc === l.n; }).length;
      return '<button class="chip" data-loc="' + esc(l.n) + '" aria-pressed="false">' + esc(l.n) + '<span class="n">' + c + '</span></button>'; }).join('') + '</div>'
...
function projTabs(id, tab) {
  ...
  return '<div class="tabs">' + t.map(function (x) { return '<a href="#/project/' + id + (x[0] ? '/' + x[0] : '') + '" class="' + ((tab || '') === x[0] ? 'on' : '') + '">' + x[1] + '</a>'; }).join('') + '</div>';
}

V.project = function (id, tab) {
  var p = P[id]; if (!p) return V.notfound();
  var x = m(p);
  var head = '<div class="wrap"><section style="padding-bottom:20px">'
    + '<p class="crumb"><a href="#/projects">Projects</a> / ' + esc(p.loc) + '</p>'
    + '<div style="margin-bottom:20px">' + pvis(p, 260) + '</div>'
    + '<p class="eyebrow">' + esc(p.dev.toUpperCase()) + ' · TG-RERA P0XXXXXXX</p>'
    ...
    + '<button class="save" ' + gated('save:' + p.id, 'Save ' + p.n + ' to your shortlist.') + ' aria-pressed="' + (isSaved(p.id) ? 'true' : 'false') + '">' + (isSaved(p.id) ? '★ Saved' : '☆ Save') + '</button>'
    + '<button class="save" data-tray="' + p.id + '" aria-pressed="' + (tray.indexOf(p.id) > -1 ? 'true' : 'false') + '">' + (tray.indexOf(p.id) > -1 ? '✓ In compare' : '⇄ Compare') + '</button></div>'
    ...
    + '<section style="padding-top:0;border-top:0">' + projTabs(id, tab);
  var body = '';

  if (!tab) {
    body = '<h2>Our verdict</h2><div class="verdict">' + ring(p.score, 88)
      + '<div><p>' + esc(p.sum) + '</p><div class="lbl">OTHO SCORE / 10 · ILLUSTRATIVE</div></div></div>'
      ...
      + AMEN.map(function (a, i) { var has = p.am.indexOf(i) > -1;
          return '<span class="chip" style="cursor:default;opacity:' + (has ? 1 : .38) + '">' + (has ? '✓ ' : '✕ ') + esc(a) + '</span>'; }).join('') + '</div>'
    ...
    body = '<h2>Floor plans</h2><p class="lede">Carpet and super built-up shown together, because one is what you buy and the other is what you pay for.</p>'
      + '<div class="plan-wrap" style="margin-bottom:14px"><div class="pcard-top" style="margin-bottom:14px">'
      + '<div><div class="meta">' + p.bhk + ' BHK · TYPICAL UNIT</div><h3>' + esc(p.n) + '</h3></div>'
      + '<div class="pmeta" style="margin:0"><span>Carpet <b>' + p.carpet.toLocaleString('en-IN') + '</b></span><span>SBA <b>' + p.sba.toLocaleString('en-IN') + '</b></span><span>Loading <b>' + x.load.toFixed(0) + '%</b></span></div></div>'
      + svgPlan(p) + '</div>'
      + '<div class="locked"><b>Download the full plan set</b><p>All unit types as a PDF, with dimensions and the RERA carpet schedule.</p>'
      + '<button class="btn" ' + gated('plan', 'Download the plan set for ' + p.n + '.') + '>Get the PDF</button></div>';
  } else if (tab === 'pricing') {
    if (user && user.verified) {
      var rows = [['Base rate on super built-up', inr(p.rate) + ' / sq ft'], ['Super built-up area', p.sba.toLocaleString('en-IN') + ' sq ft'],
        ['Base price', cr(x.total)], ['Floor rise (indicative, per floor)', inr(p.rate * 0.004 * p.sba)],
        ['Car parking (2 covered)', inr(650000)], ['Corpus and maintenance advance', inr(p.sba * 95)],
        ['GST at 5% on base', inr(x.total * 0.05)], ['Stamp duty and registration ~7.5%', inr(x.total * 0.075)]];
      body = '<h2>Pricing, broken down</h2><p class="lede">The number a sales office quotes is rarely the number you pay.</p>'
        ...
        + cr(x.total * 1.125 + 650000 + p.sba * 95 + p.rate * 0.004 * p.sba) + '</td></tr></tbody></table></div>'
        + '<p class="note">Illustrative. Confirm every line against the developer\'s cost sheet and the sale agreement.</p>';
    } else {
      body = '<h2>Pricing, broken down</h2><p class="lede">Base rate, floor rise, parking, corpus, GST, stamp duty — the whole stack, not the headline.</p>'
        + '<div class="locked"><b>Verify to see the full cost breakdown</b><p>We ask once, by OTP, so we know you are a real buyer. Your number is never passed to the developer.</p>'
    ...
    body = '<h2>Location</h2><p class="lede">Drive times measured at 9am on a weekday, not calculated from straight-line distance.</p>'
      + (l ? svgMap(l) + '<div class="stats" style="margin-top:14px">' + l.times.map(function (t) { return '<div class="stat"><b>' + esc(t[0]) + '</b><span>' + esc(t[1].toUpperCase()) + '</span></div>'; }).join('') + '</div>'
  ...
  return head + body + '</section><section class="rv">' + visitBand() + '</section></div>';
};

V.compare = function (q) {
  var a = tray[0] != null ? tray[0] : (q && q.a != null ? +q.a : 0);
  var b = tray[1] != null ? tray[1] : (a === 3 ? 0 : 3);
  return '<div class="wrap"><section><p class="eyebrow">COMPARISON ENGINE</p><h1>Compare it honestly</h1>'
    ...
    + '<div class="cmp-picks">'
    + '<div class="pick"><label for="selA">FIRST PROJECT</label><select id="selA" class="sel" aria-label="First project">' + P.map(function (p, i) { return '<option value="' + i + '"' + (i === a ? ' selected' : '') + '>' + esc(p.n) + '</option>'; }).join('') + '</select></div>'
    + '<div class="pick"><label for="selB">SECOND PROJECT</label><select id="selB" class="sel" aria-label="Second project">' + P.map(function (p, i) { return '<option value="' + i + '"' + (i === b ? ' selected' : '') + '>' + esc(p.n) + '</option>'; }).join('') + '</select></div></div>'
    + '<div class="grid g2" style="margin-bottom:18px" id="cmpVis"></div>'
...
V.locality = function (slug) {
  ...
  return '<div class="wrap"><section><p class="crumb"><a href="#/localities">Localities</a></p>'
    ...
    + (here.length ? '<section class="rv"><h2>Projects we cover here</h2><div class="grid g2">' + here.map(pcard).join('') + '</div></section>' : '') + '</div>';
};
...
V.guides = function () {
  return '<div class="wrap"><section><p class="eyebrow">GUIDES</p><h1>The mechanics, explained plainly</h1>'
    ...
    + '<div class="grid g2">' + G.map(function (g) { return '<a class="card" href="#/guides/' + g.slug + '"><div class="meta">GUIDE</div><h3>' + esc(g.t) + '</h3><p>' + esc(g.x) + '</p></a>'; }).join('') + '</div></section></div>';
};
...
V.tools = function () {
  return '<div class="wrap"><section><p class="eyebrow">CALCULATORS</p><h1>Work it out before anyone tells you</h1>'
    ...
    + '<section class="rv"><h2>Real rate on carpet</h2><p class="lede">Enter any project\'s figures. Loading factor and real rate calculate as you type.</p>'
    + calcHTML({sba:2650, carpet:1850, rate:11300}) + '</section>'
    + '<section class="rv"><h2>Loan and affordability</h2><p class="lede">Indicative only. Banks assess the project as well as you.</p>'
    + '<div class="calc"><div class="fields">'
    ...
    + '<p class="note">Lenders generally want the EMI under about 40% of net monthly income.</p></div></section></div>';
};
...
V.leads = function () {
  ...
  return '<div class="wrap"><section><p class="eyebrow">INTERNAL DEMO · NOT PUBLIC</p><h1>What you capture instead of losing</h1>'
    ...
    + '<div class="stats"><div class="stat"><b data-count="' + leads.length + '">0</b><span>VERIFIED LEADS</span></div>'
    + '<div class="stat"><b data-count="' + hot + '">0</b><span>HOT — BUYING IN 3 MONTHS</span></div>'
    + '<div class="stat"><b data-count="' + subs.length + '">0</b><span>NEWSLETTER SUBSCRIBERS</span></div>'
    + '<div class="stat"><b data-count="' + saved.length + '">0</b><span>PROJECTS SHORTLISTED</span></div></div></section>'
    + '<section class="rv">' + (leads.length
      ? '<div class="scroll"><table><thead><tr><th>Phone</th><th>Email</th><th>Profession</th><th>Timeline</th><th>Budget</th><th>Triggered by</th><th>Score</th></tr></thead><tbody>'
        + leads.slice().reverse().map(function (l) { var s = leadScore(l);
          return '<tr><td>+91 ' + esc(l.phone) + '</td><td>' + esc(l.email) + '</td><td>' + esc(l.profession) + '</td><td>' + esc(l.timeline) + '</td>'
          + '<td class="v">' + cr(l.budget) + '</td><td>' + esc(l.source) + '</td><td class="v"><span class="lead-badge ' + s[1] + '">' + s[0] + '</span></td></tr>'; }).join('') + '</tbody></table></div>'
    ...
    + '<section class="rv"><h2>Why this answers the problem</h2><div class="prose">'
    ...
    + '<p>So browsing, reviews, comparisons, guides and blog stay open. Verification is asked only where someone signals they are serious: booking a visit, downloading the plan set, unlocking the full cost breakdown, or saving a shortlist.</p>'
    + '</div></section></div>';
};
...
V.methodology = function () {
  return '<div class="wrap"><section><p class="eyebrow">HOW WE SCORE</p><h1>The method, published in full</h1>'
    ...
    + '<section class="rv"><h2>The six inputs</h2><div class="scroll"><table><thead><tr><th>Input</th><th>Weight</th><th>What we measure</th></tr></thead><tbody>'
    + SCORE.map(function (s) { return '<tr><td><strong>' + esc(s[0]) + '</strong></td><td class="v">' + esc(s[1]) + '</td><td>' + esc(s[2]) + '</td></tr>'; }).join('') + '</tbody></table></div></section>'
    + '<section class="rv"><h2>What the score is not</h2><div class="prose">'
...
V.charter = function () {
  return '<div class="wrap"><section><p class="eyebrow">EDITORIAL CHARTER</p><h1>What we will and will not publish</h1>'
    ...
    + '<section class="rv"><div class="prose"><h3>Everything negative must be a cited fact</h3>'
    + '<p>We do not publish "this project is bad." We publish "the sanctioned plan shows one clubhouse for 640 units." Every flag traces to a filing, a measurement, or an authority record.</p>'
    + '<div class="pullq">Opinion is litigable. A sourced measurement is not.</div>'
    + '<h3>Right of reply</h3><p>Developers see every finding before publication and have five working days to respond. Disputed facts carry their response alongside.</p>'
    + '<h3>Commercial separation</h3><ul><li>No developer may pay to change, soften or remove a finding</li><li>No developer may pay for placement in search or comparisons</li>'
    ...
    + '<h3>Corrections</h3><p>Errors are corrected at the top of the page, dated, with the original preserved. We do not quietly edit.</p></div></section></div>';
};
V.developers = function () {
  return '<div class="wrap"><section><p class="eyebrow">FOR DEVELOPERS</p><h1>We move inventory by making buyers confident, not by making more calls.</h1>'
    ...
    + '<section class="rv"><h2>What a mandate includes</h2><div class="grid g2">'
    ...
    + '<section class="rv"><h2>The honest part</h2><div class="prose">'
    ...
    + '<h3>How we are paid</h3><p>A commission on completed sale, on registration. No listing fees, no pay-per-lead, no paid placement.</p></div></section></div>';
};
V.about = function () {
  return '<div class="wrap"><section><p class="eyebrow">ABOUT OTHO</p><h1>We make money when you buy well, not when you buy fast.</h1>'
    ...
    + '<section class="rv"><h2>How we\'re different, concretely</h2><div class="scroll"><table><thead><tr><th>&nbsp;</th><th>A listings portal</th><th>OTHO</th></tr></thead><tbody>'
    ...
    + '<tr><td>Paid by</td><td class="v lose">Listing fees, per lead</td><td class="v win">Developer mandate on completion</td></tr></tbody></table></div></section>'
    + '<section class="rv"><h2>How we are paid</h2><div class="prose">'
    ...
    + '</div></section><section class="rv">' + visitBand() + '</section></div>';
};
V.team = function () {
  ...
  return '<div class="wrap"><section><p class="eyebrow">TEAM</p><h1>The people whose judgement this is</h1><p class="lede">A score is only worth the experience behind it.</p>'
    + '<div class="grid g2">' + t.map(function (x) { return '<div class="card pad0 lift">' + frame('PORTRAIT','Studio headshot',175) + '<div class="card-body"><h3>' + esc(x[0]) + '</h3><p>' + esc(x[1]) + '</p></div></div>'; }).join('') + '</div>'
...
V.contact = function () {
  return '<div class="wrap"><section><p class="eyebrow">CONTACT</p><h1>One conversation, with one person</h1>'
    ...
    + '<div class="grid g2"><div class="panel"><h3>Buyers</h3><ul><li>WhatsApp: +91 XXXXX XXXXX</li><li>Email: hello@otho.in</li><li>Hours: 09:00–21:00, all days</li></ul></div>'
    ...
    + '<div class="consent-row"><input type="checkbox" id="cConsent"><label for="cConsent">I agree to be contacted about this enquiry. I can withdraw at any time from <a href="#/consent" style="color:var(--brass)">consent preferences</a>.</label></div>'
...
V.careers = function () {
  return '<div class="wrap"><section><p class="eyebrow">CAREERS</p><h1>We are hiring people who would rather be right than agreeable</h1>'
    + '<p class="lede">Publishing criticism of your own clients is not for everyone.</p><div class="grid g2">'
    + [['Research analyst','Read filings, measure commutes, and defend a finding to a developer who does not like it.'],
       ['Site reviewer','Visit every project before it is scored.'],['Content producer','Two shoot days a month, roughly thirty finished pieces.'],
       ['Buyer advisor','One point of contact for a buyer, from first enquiry to registration.']].map(function (r) {
      return '<div class="card lift"><div class="meta">HYDERABAD · FULL TIME</div><h3>' + esc(r[0]) + '</h3><p>' + esc(r[1]) + '</p></div>'; }).join('')
    + '</div><p class="note">Write to careers@otho.in with something you have researched, not a covering letter.</p></section></div>';
};
V.faq = function () {
  var q = [["Do you sell my phone number?","No, and the business model does not depend on it. We earn a commission from the developer when a sale completes, so there is nothing to gain from passing your number on."],
    ["Why do I have to verify with OTP?","Only for actions that signal you are serious — booking a visit, downloading plans, unlocking the full cost breakdown. Reviews, comparisons, guides and the blog are open to everyone, no login."],
    ["How can you criticise projects you earn money from?","Because the criticism is what makes the endorsement worth anything. Every finding is a cited fact with a right of reply."],
    ...
    ["Why is your price different from the developer's?","We show the real rate on RERA carpet area alongside the advertised rate on super built-up. Same flat, different denominator."],
    ["Do you charge buyers anything?","No. Advice, comparisons, guides and site visits are free to buyers."],
    ["What if I disagree with a review?","Tell us. If you can show a figure is wrong we correct it at the top of the page, dated."],
    ["Can I buy from abroad?","Yes. The NRI article covers FEMA rules, repatriation and power of attorney."]];
  return '<div class="wrap"><section><p class="eyebrow">FAQ</p><h1>Questions we get asked</h1><div class="faq" style="margin-top:18px">'
    + q.map(function (x) { return '<details><summary>' + esc(x[0]) + '</summary><div class="ans">' + esc(x[1]) + '</div></details>'; }).join('') + '</div></section></div>';
};
V.market = function () {
  return '<div class="wrap"><section><p class="eyebrow">MARKET REPORT · AUGUST 2026</p><h1>Hyderabad, in numbers we can source</h1>'
    ...
    + '<p>The demand engine underneath is strong — Hyderabad leads the country on GCC office absorption. What has overshot is the residential supply response.</p>'
    ...
    + '</div></section><section class="rv">' + newsletter() + '</section></div>';
};
V.podcast = function () {
...
V.visit = function () {
  return '<div class="wrap"><section><p class="eyebrow">BOOK A SITE VISIT</p><h1>One conversation, with one person</h1>'
    + '<p class="lede">Pick a slot and you get a WhatsApp confirmation from a named person at OTHO. No call centre, no resold enquiry.</p>'
...
V.privacy = function () {
  return legalShell('Privacy notice','How we collect, use and protect your personal data under the Digital Personal Data Protection Act.',[
    ['h','What we collect'],['l',['Name, phone number and email when you verify or make an enquiry','Profession, buying timeline and budget, so an advisor can be useful rather than generic','The projects you view, save or compare','Device and browser information for security']],
    ['h','Why we collect it'],['p','To respond to your enquiry, arrange site visits, and send updates you asked for. We do not build advertising profiles and we do not sell data.'],
    ['h','Who we share it with'],['p','Nobody outside OTHO, other than the processors needed to run the service — hosting, messaging and CRM providers, each bound by contract. We do not pass your contact details to developers or brokers without your explicit, separate consent.'],
    ['h','Your rights'],['l',['Access the data we hold about you','Correct anything inaccurate','Erase your data','Withdraw consent at any time, as easily as it was given','Nominate someone to act for you','Complain to the Data Protection Board']],
    ['h','How long we keep it'],['p','Enquiry data for three years from your last interaction, unless you ask us to erase it sooner.'],
    ['h','Consent, stated separately'],['p','This notice is standalone. Agreeing to our terms of use is not agreement to be contacted — that consent is asked for separately, at the point of collection.']]);
};
V.consent = function () {
  var items = [['Essential service','Responding to your enquiry and arranging site visits.',true,true],
  ...
  return '<div class="wrap"><section><p class="eyebrow">CONSENT PREFERENCES</p><h1>Turn anything off, right here</h1>'
    ...
    + items.map(function (i, n) {
      return '<div style="display:flex;justify-content:space-between;align-items:center;gap:16px;background:var(--card);border:1px solid var(--line);padding:15px 17px;margin-bottom:9px;box-shadow:var(--sh-1)">'
      ...
      + ' style="width:48px;height:27px;border-radius:14px;background:' + (i[2] ? 'var(--good)' : 'var(--line)') + ';border:0;cursor:pointer;position:relative;flex:0 0 auto;transition:background .25s'
      + (i[3] ? ';opacity:.55;cursor:not-allowed' : '') + '">'
      + '<i style="position:absolute;top:3px;left:' + (i[2] ? '24px' : '3px') + ';width:21px;height:21px;border-radius:50%;background:#fff;display:block;transition:left .25s cubic-bezier(.22,.68,.36,1)"></i></button></div>'; }).join('')
    + '<div class="cta-row" style="margin-top:16px"><button class="btn" id="consSave">Save preferences</button>'
    + '<button class="btn ghost" id="consAll">Withdraw all non-essential</button><button class="btn ghost" id="consErase">Erase my data</button>'
    + '<span id="consMsg" style="font-size:13.5px;color:var(--ink-3)"></span></div></section></div>';
};
V.terms = function () {
  return legalShell('Terms of use','The rules for using this site. Plain language, no traps.',[
    ['h','What this site is'],['p','An information and advisory service for property buyers in Hyderabad. It is not a listings marketplace.'],
...
V.disclaimer = function () {
  return legalShell('Reviews disclaimer','What our scores mean and what they do not.',[
    ['h','Prototype notice'],['p','This site is currently a prototype. Project names, developers and localities are real and publicly listed. Sizes, rates, land shares, scores and verdicts are illustrative modelling for demonstration and are not published OTHO reviews or statements of fact about any project or developer.'],
    ['h','Opinion based on documented fact'],['p','In the live service, every finding traces to a RERA filing, a sanctioned plan, an authority record or a physical measurement.'],
    ...
    ['h','Point in time'],['p','A score reflects the project as assessed on the date shown. Scores are re-reviewed every six months.'],
    ['h','Right of reply'],['p','Developers see findings before publication and may respond. Demonstrated errors are corrected at the top of the page, dated.']]);
};
V.grievance = function () {
  return legalShell('Grievance officer','Who to contact if something has gone wrong with your data or a review.',[
    ['h','Data protection grievances'],['p','Under the DPDP Act you may raise a grievance about how your personal data has been handled.'],
...
var io = null;
function revealInit() {
  if (RM) { Array.prototype.forEach.call(document.querySelectorAll('.rv'), function (e) { e.classList.add('in'); }); return; }
  if (io) io.disconnect();
...
function animateRings() {
  requestAnimationFrame(function () {
    Array.prototype.forEach.call(document.querySelectorAll('.ring .val'), function (c) { c.style.strokeDashoffset = c.getAttribute('data-ring'); });
  });
}
function animateBars() {
  requestAnimationFrame(function () {
    Array.prototype.forEach.call(document.querySelectorAll('[data-bar]'), function (b) { b.style.width = b.getAttribute('data-bar') + '%'; });
  });
}
function countUp() {
  Array.prototype.forEach.call(document.querySelectorAll('[data-count]'), function (el) {
    var to = +el.getAttribute('data-count'), pre = el.getAttribute('data-prefix') || '', suf = el.getAttribute('data-suffix') || '';
    if (RM) { el.textContent = pre + to.toLocaleString('en-IN') + suf; return; }
    var t0 = null, dur = 900;
    function tick(t) {
      if (!t0) t0 = t;
...
/* ---------- compare tray ---------- */
function trayRender() {
  var t = document.getElementById('tray'), items = document.getElementById('trayItems');
  if (!tray.length) { t.classList.remove('on'); return; }
  t.classList.add('on');
  items.innerHTML = tray.map(function (id) {
    return '<span class="tray-chip">' + esc(P[id].n) + '<button data-untray="' + id + '" aria-label="Remove">×</button></span>'; }).join('');
  document.getElementById('trayGo').style.opacity = tray.length === 2 ? 1 : .45;
  document.getElementById('trayGo').textContent = tray.length === 2 ? 'Compare now' : 'Pick one more';
}
function trayToggle(id) {
  var i = tray.indexOf(id);
  if (i > -1) tray.splice(i, 1);
  else { if (tray.length >= 2) tray.shift(); tray.push(id); }
  trayRender();
  Array.prototype.forEach.call(document.querySelectorAll('[data-tray]'), function (b) {
    var on = tray.indexOf(+b.getAttribute('data-tray')) > -1;
    b.setAttribute('aria-pressed', on ? 'true' : 'false'); b.textContent = on ? '✓ In compare' : '⇄ Compare';
  });
}
...
function wireCalc() {
  ...
  function go() {
    ...
    var k = document.getElementById('keyRow');
    if (k && !RM) { k.classList.remove('flash'); void k.offsetWidth; k.classList.add('flash'); }
  }
...
function wireCompare() {
  ...
  function go() {
    ...
    document.getElementById('cmpVis').innerHTML = [A, B].map(function (p) {
      return '<div class="card pad0">' + pvis(p, 150) + '<div class="card-body"><div class="pcard-top"><div><h3>' + esc(p.n) + '</h3>'
      + '<div class="meta">' + esc(p.dev) + ' · ' + esc(p.loc) + '</div></div>' + ring(p.score, 52) + '</div></div></div>'; }).join('');
    var rows = [['Advertised rate', inr(A.rate) + ' / sq ft', inr(B.rate) + ' / sq ft', A.rate, B.rate, -1],
    ...
    rows.forEach(function (r) {
      ...
      var cls = ['', ''], mx = Math.max(r[3], r[4]) || 1;
      if (r[5] !== 0 && r[3] !== r[4]) { var ab = r[5] === -1 ? r[3] < r[4] : r[3] > r[4]; cls = ab ? ['win','lose'] : ['lose','win']; }
      [[r[1], cls[0], r[3]], [r[2], cls[1], r[4]]].forEach(function (cc) {
        var td = document.createElement('td'); td.className = 'v ' + cc[1];
        td.innerHTML = esc(cc[0]) + (r[5] !== 0 ? '<div class="cbar"><i data-bar="' + Math.round(cc[2] / mx * 100) + '" style="background:' + (cc[1] === 'win' ? 'var(--good)' : 'var(--bad)') + '"></i></div>' : '');
        tr.appendChild(td); });
      body.appendChild(tr);
    });
    animateRings(); animateBars(); photoInit(document.getElementById('cmpVis'));
  }
...
function wireSearch() {
  ...
  var bhk = document.getElementById('qBhk'), st = document.getElementById('qStat'), so = document.getElementById('qSort'),
      bud = document.getElementById('qBud'), budV = document.getElementById('qBudV'), amBox = document.getElementById('qAm'), tmr = null;
  function sel() { return Array.prototype.filter.call(amBox.children, function (c) { return c.getAttribute('aria-pressed') === 'true'; }).map(function (c) { return +c.getAttribute('data-am'); }); }
  function results() {
    var want = sel();
  ...
  function counts() {
    Array.prototype.forEach.call(amBox.children, function (c) {
      var i = +c.getAttribute('data-am');
      c.querySelector('.n').textContent = P.filter(function (p) { return p.am.indexOf(i) > -1; }).length;
    });
  }
  function go(instant) {
    budV.textContent = '₹' + (+bud.value / 1e7).toFixed(2) + ' Cr';
    var out = document.getElementById('qList');
    if (!instant && !RM) { out.innerHTML = '<div class="skel"></div><div class="skel"></div>'; }
    clearTimeout(tmr);
  ...
  bud.addEventListener('input', function () { go(true); });
...
function wireVisit() {
  ...
  document.getElementById('bookBtn').addEventListener('click', function () {
    ...
    document.getElementById('bookMsg').textContent = 'Confirmed for ' + chosen + '. Prototype — a live build sends WhatsApp confirmation to +91 ' + user.phone + '.';
  });
}
function wireNewsletter() {
  ...
  f.addEventListener('submit', function (e) {
    ...
    msg.textContent = 'Subscribed. ' + v + ' will get the next monthly note.';
    document.getElementById('nlEmail').value = ''; toast('Subscribed to the monthly market note.');
  });
}
function wireMisc() {
  ...
  if (cb) cb.addEventListener('click', function () {
    ...
    document.getElementById('cMsgOut').textContent = !n ? 'Add your name first.'
      : (!ok ? 'Please tick consent — we cannot contact you without it.' : 'Prototype — a live build writes this to the CRM with a consent timestamp.');
  });
  var cs = document.getElementById('consSave');
  if (cs) cs.addEventListener('click', function () { document.getElementById('consMsg').textContent = 'Preferences saved.'; toast('Preferences saved.'); });
  ...
  if (ce) ce.addEventListener('click', function () {
    user = null; leads = []; subs = []; saved = []; tray = [];
    ls.set('otho_user', null); ls.set('otho_leads', []); ls.set('otho_subs', []); ls.set('otho_saved', []);
    updateSl(); trayRender(); document.getElementById('consMsg').textContent = 'Everything erased from this device.';
  });
}
...
function render() {
  ...
  var top = s[0] || '';
  var moreRoutes = { tools: 1, podcast: 1, developers: 1, about: 1, faq: 1, contact: 1 };
  ...
  var drop = document.getElementById('navDrop'), moreBtn = document.getElementById('navMore');
  if (drop && moreBtn) {
    drop.classList.toggle('open', false);
    moreBtn.setAttribute('aria-expanded', 'false');
    moreBtn.classList.toggle('on', !!moreRoutes[top]);
  }
...
document.addEventListener('click', function (e) {
  ...
  if (g) { e.preventDefault(); requireUser(g.getAttribute('data-gate'), g.getAttribute('data-glabel')); return; }
  var t = e.target.closest('[data-tray]');
  if (t) { e.preventDefault(); trayToggle(+t.getAttribute('data-tray')); return; }
  var u = e.target.closest('[data-untray]');
  if (u) { trayToggle(+u.getAttribute('data-untray')); return; }
  var f = e.target.closest('[data-gal]');
  if (f && gal.length && f.getAttribute('data-gal') !== '') { openLb(+f.getAttribute('data-gal')); return; }
  var sw = e.target.closest('.sw');
  if (sw && !sw.disabled) {
    var on = sw.getAttribute('aria-pressed') === 'true';
    sw.setAttribute('aria-pressed', on ? 'false' : 'true');
    sw.style.background = on ? 'var(--line)' : 'var(--good)';
    sw.querySelector('i').style.left = on ? '3px' : '24px';
  }
});
document.getElementById('trayClear').addEventListener('click', function () { tray = []; trayRender(); render(); });
document.getElementById('trayGo').addEventListener('click', function (e) { if (tray.length !== 2) { e.preventDefault(); toast('Pick two projects to compare.'); } });
...
document.addEventListener('keydown', function (e) {
  if (e.key === 'Escape') { closeModal(); document.getElementById('lb').classList.remove('on'); }
...
window.addEventListener('scroll', function () {
  ...
  document.querySelector('nav').classList.toggle('stuck', y > 8);
  document.getElementById('top').classList.toggle('on', y > 700);
}, { passive: true });
document.getElementById('top').addEventListener('click', function () { window.scrollTo({ top: 0, behavior: RM ? 'auto' : 'smooth' }); });
...
(function themeInit() {
  ...
  if (btn) btn.addEventListener('click', function () {
    ...
    try { localStorage.setItem('otho_theme', next); } catch (e) {}
    sync();
  });
})();

(function navDropInit() {
  var drop = document.getElementById('navDrop'), more = document.getElementById('navMore');
  if (!drop || !more) return;
